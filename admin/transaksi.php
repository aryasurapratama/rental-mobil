            ...
            <div class="row">
              <div class="col-lg-12">
                <table class="table">
                  ...
                  <tbody>
                    <?php
                    $result = $mysqli->query("SELECT * FROM tb_sewa as s INNER JOIN mobil as m ON s.id_mobil=m.id_mobil INNER JOIN user as us ON us.id_user=s.id_user INNER JOIN tb_detail_sewa as tbs ON tbs.id_sewa=s.id_sewa INNER JOIN pembayaran as p ON p.id_detail_sewa=tbs.id_detail_sewa WHERE p.status='BELUM KONFIRMASI'");
                    $i = 1;
                    if($result === FALSE){
                      die(mysql_error());
                    }
                    ...
                    if($result){
                      while($obj = $result->fetch_object()) {
                        ...
                        echo 		'<td><img style="width:100px;" src="../asset_ktp/'.$obj->bukti_ktp.'"/></td>';
                          echo 		'<td>+'.$obj->no_hp.'</td>';
                        echo 		'<td>
                                  <a href="backend_admin/konfirmasi.php?action=add&id='.$obj->id_pembayaran.'"><em class="fa fa-lg fa-check-circle"></em> KONFIRMASI</a>
                                </td>';
                        echo 	'</tr>';
                      }
                    }
